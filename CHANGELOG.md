...
### 🔧 修复
- **国语第三版可选包装层兼容**：明鏡国語辞典 第三版词条即使没有 `div[data-sc-dic-item]` 外层包装，也能根据词典名称和 `data-sc-body` 正确识别、置顶并高亮下划线释义，修复「引く」等卡片完全不重排的问题。
- **新版扁平 DOM 布局兼容**：支持无 `.gloss-image*` class 的真实 gaiji 图片并压缩到 `1em`；置顶时保留原分区标题与首个 `level0` 的相邻关系，同时避免把全词条“書き分け”和类语表误并入移动分区。

### 🧪 测试验证
- ✅ 新增 `引く.html` 和对应 `test-output/引く.html` 快照，覆盖缺少 `data-sc-dic-item` 包装层的 DOM 结构
- ✅ 验证目标 `52067-D035` 保持在所属 `level0` 分区 `52067-D031` 内，并与例句 `52067-5052` 一起置顶
- ✅ 验证 classless gaiji 尺寸、原分区标题顺序以及词条尾部说明位置
- ✅ 刷新 `付き.html` 的白天与夜间快照，确认类语表保留在词条末尾
- ✅ 18 项自动验证全部通过
- ✅ 使用 Chrome DevTools 在 `390×844` 移动端视口检查高亮、`level0` / `level1` 顺序、gaiji 尺寸、后续例句和夜间颜色切换
